...
const PREF_LABELS: Record<PrefKey, { label: string; description: string }> = {
  jackpotAlerts:   { label: 'Jackpot Alerts', description: 'Get notified when jackpots hit new highs' },
  tosChanges:      { label: 'TOS Changes', description: 'Plain-English alerts when platform terms change' },
...
export function NotificationPreferences() {
  ...
  const handleSubscribePush = async () => {
    ...
    try {
      ...
      await api.notifications.subscribePush(JSON.parse(JSON.stringify(subscription)) as Record<string, unknown>)
    ...
    <div className="space-y-4">
      ...
      <div className="glass-card rounded-2xl p-4 space-y-3">
        ...
        {pushStatus === 'subscribed' ? (
          ...
          <button
            ...
            className={cn(
              'flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold transition-all press-scale',
              'bg-brand-500/15 text-brand-300 ring-1 ring-brand-500/30 hover:bg-brand-500/25',
              pushStatus === 'subscribing' && 'opacity-60 cursor-wait',
            )}
          >
      ...
      <div className="flex items-center gap-3">
        <button
          ...
          disabled={isSaving}
          className={cn(
            'flex items-center gap-2 px-5 py-2 rounded-xl text-sm font-semibold transition-all press-scale',
