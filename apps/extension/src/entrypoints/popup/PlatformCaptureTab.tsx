...
export default function PlatformCaptureTab() {
  ...
  async function withActiveTab<T>(
    action: (tabId: number, url: string | null) => Promise<T>
  ): Promise<T> {
    ...
    runAction(async () => {
      await withActiveTab(async (tabId) => {
        await chrome.scripting.executeScript({
          target: { tabId },
          world: MAIN_WORLD,
          func: installPlatformCapture,
        })
        const [{ result }] = await chrome.scripting.executeScript({
          target: { tabId },
          world: MAIN_WORLD,
          func: readPlatformCapture,
        })
  ...
  return (
    <div className="space-y-3 p-4">
      ...
      <ol className="list-decimal space-y-1 pl-4 text-xs text-gray-600">
        ...
        <li>
          Click <strong>Start Capture</strong>.
        </li>
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <button
          ...
          disabled={busy}
          className="rounded-lg bg-gray-100 px-3 py-2 text-xs font-semibold text-gray-800 disabled:opacity-60"
        >
          <span className="inline-flex items-center gap-1">
      ...
      {tabUrl && (
        <div className="break-all rounded-lg bg-gray-50 p-2 text-[11px] text-gray-600">
          Active tab: {tabUrl}
        </div>
      )}
      {error && (
      ...
      {copied && (
        <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-2 text-[11px] text-emerald-700">
          Report copied. Paste it back into chat.
        </div>
      )}
      ...
      <div className="grid grid-cols-3 gap-2 text-center text-xs">
        <div className="rounded-lg bg-gray-50 p-2">
          ...
          <div className="font-semibold text-gray-900">{snapshot?.requests.length ?? 0}</div>
        </div>
        <div className="rounded-lg bg-gray-50 p-2">
          <div className="text-gray-500">Balance</div>
          <div className="font-semibold text-gray-900">{analysis.balanceCandidates.length}</div>
        </div>
        <div className="rounded-lg bg-gray-50 p-2">
          <div className="text-gray-500">Spins</div>
          <div className="font-semibold text-gray-900">{analysis.transactionCandidates.length}</div>
        </div>
      </div>
      ...
      <CandidateSection
        ...
        candidates={analysis.transactionCandidates}
      />
    </div>
  )
}
...
function CandidateSection({
  ...
  title: string
  candidates: ReturnType<typeof analyzePlatformCapture>['balanceCandidates']
}) {
  return (
    <div className="space-y-2">
      ...
      {candidates.length === 0 ? (
        ...
        candidates.slice(0, 2).map((candidate) => (
          <div
            key={candidate.id}
            className="space-y-1 rounded-lg border border-gray-200 p-2 text-[11px] text-gray-700"
          >
            <div>
              <strong>URL:</strong> <span className="break-all">{candidate.url}</span>
            </div>
            ...
            <div className="overflow-x-auto rounded bg-gray-50 p-2">
              <pre>{JSON.stringify(candidate.suggestedPaths, null, 2)}</pre>
            </div>
          </div>
        ))
      )}
