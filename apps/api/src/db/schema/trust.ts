...
export const trustIndexScores = pgTable(
  'trust_index_scores',
  {
    ...
    redemptionRejectionRateScore: numeric('redemption_rejection_rate_score', {
      precision: 5,
      scale: 2,
    }).notNull(),
    ...
    communitySatisfactionScore: numeric('community_satisfaction_score', {
      precision: 5,
      scale: 2,
    }).notNull(),
    supportResponsivenessScore: numeric('support_responsiveness_score', {
      precision: 5,
      scale: 2,
    }).notNull(),
    regulatoryStandingScore: numeric('regulatory_standing_score', {
      precision: 5,
      scale: 2,
    }).notNull(),
  ...
  (table) => ({
    platformTimeIdx: index('trust_index_scores_platform_time_idx').on(
      table.platformId,
      table.calculatedAt
    ),
...
export const trustIndexAlerts = pgTable(
  'trust_index_alerts',
  {
    id: uuid('id').primaryKey().defaultRandom(),
  ...
  (table) => ({
    ...
    userPlatformUnique: unique('trust_index_alerts_user_platform_unique').on(
      table.userId,
      table.platformId
    ),
...
export const platformRatings = pgTable(
  'platform_ratings',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    platformId: uuid('platform_id').notNull(),
  ...
  (table) => ({
    ...
    userPlatformUnique: unique('platform_ratings_user_platform_unique').on(
      table.userId,
      table.platformId
    ),
...
export const platformBonuses = pgTable(
  'platform_bonuses',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    platformId: uuid('platform_id').notNull(),
...
export const platformCommunitySignals = pgTable(
  'platform_community_signals',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    platformId: uuid('platform_id').notNull(),
