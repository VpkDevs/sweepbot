...
function granularityExpr(granularity: Granularity): ReturnType<typeof sql> {
  switch (granularity) {
    case 'week':
      return sql`DATE_TRUNC('week',  ${sessions.startedAt})::date`
    case 'month':
      return sql`DATE_TRUNC('month', ${sessions.startedAt})::date`
    default:
...
export const analyticsRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get(
    ...
    async (request) => {
      ...
      // ── Automated insights ──
      const hourData = byHourRaw.map((r) => ({
        slot: r.hour,
        avg_rtp: r.avg_rtp,
        session_count: r.session_count,
      }))
      const dowData = byDowRaw.map((r) => ({
        slot: r.dow,
        avg_rtp: r.avg_rtp,
        session_count: r.session_count,
      }))
  ...
  fastify.get(
    ...
    async (request) => {
      ...
      const startDate = new Date(Date.now() - days * 24 * 60 * 60 * 1_000)
      const conditions = [eq(transactions.userId, userId), gte(transactions.timestamp, startDate)]
      ...
      const byGameRaw = await db
        ...
        .groupBy(sql`t.game_id`, sql`g.name`, sql`gi.bonus_frequency`)
  ...
  fastify.get('/insights', async (request) => {
    ...
    const [records] = await db
      .select({
        ...
        worst_session_rtp: sql<
          number | null
        >`MIN(CASE WHEN ${sessions.spinCount} > 20 THEN ${sessions.rtp} ELSE NULL END)`,
    ...
    const [hourBest] = await db
      ...
      .having(sql`COUNT(*) >= 3`)
      .orderBy(
        desc(sql`(SUM(${sessions.totalWon}) / NULLIF(SUM(${sessions.totalWagered}), 0)) * 100`)
      )
      .limit(1)
  ...
  fastify.get(
    ...
    async (request, reply) => {
      ...
      // Serialise to CSV
      const header =
        'date,platform,game,duration_minutes,total_wagered_sc,total_won_sc,net_result_sc,rtp_pct,spin_count'
