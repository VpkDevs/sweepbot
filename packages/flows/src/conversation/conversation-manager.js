...
export class ConversationManager {
     ...
     */
    async startConversation(userId, sessionId, initialFlowDescription) {
        const state = {
            userId,
            sessionId,
            currentFlow: {},
     ...
     */
    async continue(conversationId, userMessage) {
     ...
     */
    detectUpdateIntent(userMessage) {
     ...
     */
    async applyFlowRefinement(state, userMessage, _keywords) {
     ...
     */
    generateFollowUpQuestions(flow) {
        const questions = [];
     ...
     */
    getMissingFields(flow) {
        const missing = [];
        if (!flow.trigger)
            missing.push('Trigger (when should it run?)');
        if (!flow.rootNode)
            missing.push('Actions (what should it do?)');
        if (!flow.responsiblePlayGuardrails || flow.responsiblePlayGuardrails.length === 0)
     ...
     */
    answerFlowQuestion(userMessage, _flow) {
        const lowerMessage = userMessage.toLowerCase();
     ...
     */
    generateConfirmationCard(flow) {
        let card = '```\n';
        // Trigger
        if (flow.trigger?.type === 'scheduled') {
            const scheduled = flow.trigger;
