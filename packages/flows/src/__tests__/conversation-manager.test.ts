...
describe('ConversationManager', () => {
  ...
  it('should start a conversation and interpret initial description', async () => {
    const state = await conv.startConversation(
      'userA',
      'sess1',
      'Every day at 5 PM, open Chumba and claim bonus'
    )
  ...
  it('should confirm and return flow when complete', async () => {
    ...
    state.currentFlow.responsiblePlayGuardrails = [
      { type: 'max_duration', value: 0, source: 'system_default', overridable: true },
    ]
