...
export class EntityRecognizer {
  private platformAliases: Map<string, string> = new Map([
  ...
  extractGames(text: string): GameEntity[] {
    ...
    return games.filter((g) => {
      if (seen.has(g.normalized)) return false
  ...
  extractActions(text: string): ActionEntity[] {
    ...
    const actionKeywords: { keywords: string[]; action: FlowActionType; confidence: number }[] = [
   ...
   */
  private extractVariables(text: string): VariableEntity[] {
   ...
   */
  private normalizeHour(hour: number, meridiem?: string): number {
    if (!meridiem) return hour // Already in 24-hour format
