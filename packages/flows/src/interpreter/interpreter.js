...
export class FlowInterpreter {
    ...
    classifyIntent(text, entities) {
        ...
        if (lowerText.includes('bonus'))
            return 'bonus_collection';
        if (lowerText.includes('spin') || lowerText.includes('play'))
            return 'play_strategy';
        if (lowerText.includes('cash out') || lowerText.includes('redeem'))
            return 'redemption';
        if (lowerText.includes('every') || lowerText.includes('daily'))
            return 'recurring_routine';
        if (lowerText.includes('jackpot'))
            return 'jackpot_hunting';
     ...
     */
    buildFlowAST(entities, intent, rawText) {
        const rootSteps = [];
     ...
     */
    buildOpenPlatformNode(platform) {
        return {
            type: 'action',
            id: this.generateId(),
     ...
     */
    extractLoopCondition(text, entities) {
     ...
     */
    buildSpinLoopNode(condition, entities) {
        const betAmount = this.extractBetAmount(entities);
        return {
            type: 'loop',
            id: this.generateId(),
     ...
     */
    validateResponsiblePlay(flowNode, userId) {
        const guardrails = [];
     ...
     */
    calculateConfidence(entities, intent) {
