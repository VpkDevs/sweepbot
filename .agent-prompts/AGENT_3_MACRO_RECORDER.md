...
export interface RecordedAction {
  ...
  actionType: 'login' | 'claim_bonus' | 'spin' | 'navigate' | 'check_balance' | 'cash_out' | 'unknown';
...
export class MacroRecorder {
  ...
  private classifyAction(element: HTMLElement): { type: RecordedAction['actionType'], confidence: number } {
  ...
  private generateSelector(element: HTMLElement): string {
    ...
    const classes = Array.from(element.classList).filter(c => !c.match(/^(active|hover|focus)/)).join('.');
    if (classes) {
