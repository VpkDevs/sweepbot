...
trialExpirationQueue.add('check-expired-trials', {}, {
  repeat: { pattern: '0 0 * * *' } // Cron: midnight daily
});

const worker = new Worker('trial-expiration', async (job) => {
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Your {previousStreak}-day streak has ended. Start a new streak by tracking a session today!
          </DialogDescription>
        </DialogHeader>
        
        <div className="py-4">
          <p className="text-sm text-gray-600">
            💡 <strong>Pro tip:</strong> Pro subscribers get 3 streak freeze credits per month to protect their streaks.
          </p>
        </div>
...
export async function sendPushNotification(
  ...
  const expiredIndexes = results
    .map((result, idx) => result.status === 'rejected' && result.reason.statusCode === 410 ? idx : -1)
    .filter(idx => idx !== -1);
...
app.post('/notifications/subscribe', async (req, res) => {
  ...
  }).onConflict('endpoint').merge(); // Update if exists
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        >
          {isSubscribed ? (
            <><BellOff className="h-4 w-4 mr-2" /> Disable</>
          ) : (
            <><Bell className="h-4 w-4 mr-2" /> Enable</>
          )}
        </Button>
      </div>
        ...
        <div className="space-y-4 border-t pt-4">
          <div className="flex items-center justify-between">
            ...
            <Switch
              ...
              onCheckedChange={(checked) =>
                updatePreferences.mutate({ jackpot_alerts: checked })
              }
            />
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <Switch
              ...
              onCheckedChange={(checked) =>
                updatePreferences.mutate({ tos_changes: checked })
              }
            />
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <Switch
              ...
              onCheckedChange={(checked) =>
                updatePreferences.mutate({ platform_outages: checked })
              }
            />
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <Switch
              ...
              onCheckedChange={(checked) =>
                updatePreferences.mutate({ flow_errors: checked })
              }
            />
          </div>
        </div>
  ...
  return (
    <div className="flex items-start gap-3 p-3 bg-blue-50 rounded-lg">
      ...
      <div className="flex-1">
        ...
        <p className="text-xs text-gray-500 mt-1">{duration}s</p>
      </div>
