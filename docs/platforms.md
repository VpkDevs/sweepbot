...
**Premium Currency** (e.g., "Sweeps Coins") - redeemable for prizes
...
✅ Base all information on your knowledge cutoff date
❌ DO NOT fabricate, guess, or include unverified information
❌ If uncertain about any detail, explicitly mark as "Info N/A" rather than guessing

### Scope & Focus

**Primary focus**: NEW and emerging sweepstakes casinos (launched within last 2-3 years preferred)
**Rationale**: Newer platforms often feature more competitive bonuses, modern games, and responsive websites
**Target quantity**: 800-1500 verified platforms
Include well-established platforms only if space permits after covering newer entrants
...
**Website URL**: Format as casinoname.com (no https:// prefix)
...
**First Purchase Bonus**: Best value on initial GC purchase (e.g., "Buy 10,000 GC, get 25 SC")
**Second Purchase Bonus**: Best value on second purchase (e.g., "Buy 25,000 GC, get 75 SC")
**Important**:
If specific bonus amounts are unknown, use "Info N/A" for that field
If bonuses vary/rotate, note "Varies - check site"
...
Use "Info N/A" if specific currency names are unknown

### 4. Game Library

**Game Selection**: Quantity and types (e.g., "300+ slots, 15 table games, 5 live dealer")
**Fishing Games**: Y/N/Unknown
**Alternative Games**: Y/N/Unknown (crash, plinko, mines, chicken crossing, etc.)
Use ranges or "100+" style estimates if exact numbers are unavailable
Slot providers (pragmatic play, etc.)
Slot mechanics present (megaways, true ways, immortal ways, hold and win, etc.)
...
**Unique Features**: If any, what utterly unique aspects does this platform have
**Key Features**: 2-3 concise bullet points highlighting unique aspects
...
The number of verified platforms you're including
...
Sort alphabetically by casino name
Use consistent abbreviations (GC for Gold Coins, SC for Sweeps Coins)
Keep cells concise but informative
Use • bullet points for multiple items within cells
Use "Info N/A" for unknown information
...
Current promotional offers directly on platform websites
...
**Terminology**: Consistent throughout (standardize all abbreviations)
**Uncertainty handling**: Always use "Info N/A" or "Varies - check site" rather than guessing
**Formatting**: Clean, scannable markdown that renders properly
**Transparency**: If you have limited information on newer platforms, acknowledge this

## Final Checklist Before Submitting

All included casinos are verified sweepstakes platforms from your training data (not real-money gambling sites)
No fabricated or guessed information included
Newer/emerging platforms prioritized where verifiable
Table is properly formatted with correct markdown syntax
Disclaimer included with correct knowledge cutoff date
Consistent terminology and abbreviations used throughout
All uncertain information clearly marked as "Info N/A"
Introductory note explains any data limitations
...
**Multi‑axis filtering and comparison**: allow side‑by‑side comparisons of up to 4 operators showing **bonuses, expected cash value, redemption minimums, average payout time, and complaint volume**.
**Smart search**: natural language queries (e.g., “redeems within 48 hours in Texas, no KYC”) and saved search alerts.

#### Ratings, Reviews, and Trust Signals

**Verified reviews pipeline**: email/phone verification, small on‑site micro‑transaction verification, or identity‑verified reviewers; each review includes structured fields (redemption time, payout method, support responsiveness). sweepcasinos.com
**Editorial audits**: staff testers play and redeem on each operator on a regular cadence; publish audit reports and a **composite trust score** combining editorial results, user reviews, complaint counts, and third‑party audits.
**Incident timeline and public complaints board**: users and regulators can see open complaints, operator responses, and resolution status.

#### Offers, Bonuses, and Value Tools

**Real‑time bonus aggregator** with historical price charts and **expected value calculators** that convert Gold/Sweeps into estimated cash value given operator redemption rules. sweepsranking.com
**Exclusive deals and tracking**: time‑limited coupons, first‑purchase comparisons, and alerting when a previously seen offer returns.

#### Education and Legal Clarity
...
**Escrowed Verification Service**: partner with a regulated payments/escrow provider to independently verify a sample of redemptions and publish audit reports showing proof of payout rates.
**Blockchain Proof‑of‑Payout**: privacy‑preserving hashes of payout events recorded on a public ledger to provide immutable proof without exposing PII.
**AI Dispute Resolution**: automated triage that collects evidence, suggests next steps, and mediates between player and operator; escalate to human arbitrators when needed.
**Personalized Sweeps Coach**: AI assistant that recommends offers and games based on bankroll, risk tolerance, and historical win patterns; includes bankroll management and expected value guidance.
...
**Verification layer**: human audits, escrow partner inputs, and blockchain anchoring.
**Core services**: search index, personalization engine, review engine, offers engine, and analytics.
**APIs**: public listing API, partner webhooks, and private partner endpoints.
**Frontend**: PWA + native apps; server‑side rendering for SEO; CDN for global performance.

#### Key data entities (minimal)

**Operator**: id; name; domains; corporate owner; launch_date; states_allowed; contact_info; verification_status.
**Offer**: id; operator_id; type; value; start_date; end_date; terms_snapshot_id.
**Review**: id; operator_id; user_id; verified_flag; rating_components (payout_speed, support, fairness); evidence_attachments_hash.
**PayoutEvent**: id; operator_id; anonymized_user_hash; request_timestamp; processed_timestamp; amount; method; verification_status.
**Incident**: id; operator_id; description; status; evidence_hashes; resolution.

#### Suggested API endpoints (one-line)

GET /operators?state=TX&min_redemption=50
GET /operators/{id}/offers
POST /operators/{id}/claim (operator claim + verification workflow)
...
**Verification ops**: small QA team to run periodic redemption tests and audit operator claims.
**Community moderation**: mix of human moderators and ML filters for fake reviews and abusive content.

#### Compliance and legal

**State legality monitoring** with conservative labeling (e.g., “Not available in your state”) until verified.
**KYC/AML guidance**: explain when operators may require identity checks; do not store PII unless necessary and encrypted.
**Third‑party audits**: publish audit reports and partner with consumer protection orgs to increase credibility.
...
**Verification & Trust (4–10 months)**: verified reviews, editorial audits, operator claim portal, and incident board.
**Advanced features (10–18 months)**: live payout tracker, partner escrow verification, APIs, PWA/native apps.
**Visionary (18–36 months)**: blockchain proof‑of‑payout, AI dispute resolution, cross‑operator tournaments.

#### Key metrics

**Coverage**: % of active operators listed.
**Trust**: verified review ratio; average trust score.
**Engagement**: DAU/MAU; time on legal pages.
**Conversion**: click‑through to redemption; affiliate revenue per user.
**Resolution**: average complaint resolution time.

#### Major risks and mitigations

**Fake reviews and manipulation** — use multi‑factor verification, ML detection, and review weighting.
**Operator misrepresentation** — require evidence for claims; publish audit results and takedown procedures.
...
**Prioritized MVP roadmap with resource estimates and sprint milestones**, or
